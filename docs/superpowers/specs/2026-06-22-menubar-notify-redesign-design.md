...
| 决策点 | 选择 |
|---|---|
| 呈现方式 | 新通知先弹横幅，点击横幅/折叠行/铃铛展开为完整面板 |
...
bannerStack  →  铃铛下方：≤3 条横幅 + 折叠行        小窗口，右对齐铃铛
panel        →  铃铛下方：完整消息中心               大窗口，右对齐铃铛
```

### 4.2 锚点迁移

- 现：`windowFrame` 由 `screenRect.midX/maxY`（刘海居中）计算。
- 改：由**铃铛图标的屏幕坐标**计算——窗口右边缘对齐铃铛右边缘，顶边贴铃铛下沿，向下展开。
...
手动清单：
1. 单条 / 多条(≤3 / >3)通知 → 横幅与折叠行计数正确。
2. 可操作横幅不自动消失；普通横幅按时消失。
3. 点横幅按钮触发回调并显示结果通知（沿用现有链路）。
4. 铃铛点击开/关 panel；点横幅/折叠行展开；点外部/Esc 收起。
5. 多屏切换后定位仍锚定铃铛；启动早期取不到坐标的回退路径。
