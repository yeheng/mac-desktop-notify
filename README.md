...
- 🖥️ **菜单栏横幅/面板 UI** — 新通知先以横幅形式从屏幕右上角弹出（与 macOS 原生通知一致）；点击菜单栏铃铛在图标下方展开完整消息中心，点击横幅也可打开消息中心
...
回调结果同时会以新通知的形式显示在消息中心面板中（成功 = 绿色通知，失败 = 红色通知，5 秒后自动消失）。
...
└── MacIsland/
    ├── BannerQueue.swift               # 横幅队列纯逻辑（可见/溢出）
    ├── BannerCardView.swift            # 单条横幅视图
    ├── BannerStackView.swift           # 横幅堆叠 + 折叠行
    ├── ViewHeightKey.swift             # 内容高度测量 PreferenceKey
    ├── Ext+NSStatusItem.swift          # 铃铛屏幕坐标
