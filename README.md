...
- 📂 **历史分组聚合** — 同 `group` 的历史消息折叠为一行，点击展开逐条查看，未读数自动累计
- 🖱️ **右键菜单** — 面板右键即可打开/收起、静默 1 小时、清除全部或进入设置
- ⌨️ **键盘操作** — 指针在面板上时 `⌘1`/`⌘2`/`⌘3` 直接触发对应操作按钮，审批流无需点击
- 📜 **消息列表** — 当前消息、待显示队列和历史（最多 50 条）同屏展示，打开面板时第一条历史默认展开正文，标题旁一键删除单条，长标题摘要栏自动跑马灯滚动
...
| 右键点击刘海/面板 | 快捷菜单：打开或收起面板、静默 1 小时（可取消）、清除全部消息、设置… |
| `⌘1` / `⌘2` / `⌘3` | 触发当前消息的第 1/2/3 个操作按钮（指针在面板/刘海区域上时生效，不抢占其他 App 的快捷键） |
| 点击历史消息 | 就地展开/收起渲染后的 Markdown 正文和操作按钮；每次打开面板时，第一条历史消息（或分组的最新一条）默认展开，点 `>` 收起 |
| 历史消息标题右侧的 🗑 | 从历史中删除单条消息，无需先展开正文 |
