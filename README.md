...
| `⌘,` | 打开设置 |
| `⌘Delete` | 清除消息 |
| `Esc` | 收起面板（仅当指针停留在面板或刘海区域时生效，避免在 vim 等 App 中误触） |
| 点击 `×` 按钮 | 清除当前消息并收起面板（指针离开刘海区域后才会再次悬停展开） |
| 点击垃圾桶按钮 | 清空全部消息（弹出确认，防止误清空历史） |

快捷键默认只在本 App 激活时生效；在「设置 → 快捷键」中开启「在任意 App 中启用快捷键」后全局生效。注意全局模式下 `⌘,` 与多数 App 的「设置」、`⌘⇧N` 与 Finder「新建文件夹」、`⌘Delete` 与「移到废纸篓」冲突。
