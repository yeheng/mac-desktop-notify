...
启动后点击菜单栏铃铛 → 右键菜单「设置…」，或在消息中心面板点击齿轮按钮，打开设置面板：

| 分区 | 配置项 | 生效方式 |
|------|--------|----------|
| **服务** | API 端口、认证 Token | 改后点「立即应用」重建服务 |
| **Banner** | 启用开关、默认超时、最大显示数、显示位置 | 实时生效 |
| **通知** | 默认超时、历史保留数 | 实时生效 |
| **外观** | Banner 宽度、圆角 | 实时生效 |

设置持久化到 `UserDefaults`，重启后保留。首次运行时回退到环境变量默认值。

#### 环境变量（首次启动默认值）

环境变量仅在首次启动（UserDefaults 无记录时）作为默认值，之后以设置面板为准：
...
| 操作 | 说明 |
|------|------|
| 点击菜单栏铃铛图标 | 打开/关闭消息中心面板 |
| 点击面板右上角 ✕ | 关闭面板 |
| 点击面板外部 | 自动收起面板 |
| `Esc` | 关闭面板 |
| 点击 banner 右上角 ✕ | 关闭单条 banner |
| 向右滑动 banner | 关闭单条 banner |
| 点击 banner 卡片 | 展开/折叠（分组通知展开显示组内全部） |
| 点击操作按钮 | 触发回调，结果**原地替换**横幅内容 |
| 双击通知卡片 | 复制正文到剪贴板 |
| 右键通知卡片 | 上下文菜单（复制标题/正文/全部、删除） |
| 单条删除 | 显示撤销 toast（4s 内可恢复） |
| 点击「清空」 | 清空全部通知（二次确认） |
| 通知中心搜索框 | 按标题/正文实时搜索 |
| 通知中心类型 chip | 按 info/success/warning/error 过滤 |
| `↑` `↓` `Enter` `⌫` | 通知中心键盘导航（选择/触发/删除） |
