...
extension DynamicNotch {
    ...
    func _compact(on screen: NSScreen = NSScreen.screens[0], skipHide: Bool) async {
        ...
        // local patch: comment only, behaviour unchanged. Floating screens have no
        // pill to reach - this hides and reports nothing, so a caller that wanted
        // a summary gets a silent no-op. The app avoids the branch by asking
        // `screen.hasNotch` first (`SummaryRouting`), and because it passes an
        // explicit `.notch` style this never runs from this app at all.
