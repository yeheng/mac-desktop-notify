...
extension NSScreen {
    ...
    // local patch: public so the app asks the kit instead of re-deriving the
    // same test. `hasNotch` decides whether a compact pill can be drawn at all
    // (the app's `SummaryRouting`), `notchSize` is the measured notch rect the
    // app's trigger zones and mini bar are built from.
